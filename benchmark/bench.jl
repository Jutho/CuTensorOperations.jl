...
function mpsbench(T = Float64; D = 256, d = 2, m = 8)
    ...
    b4 = @benchmark CuArrays.@sync @cutensor x4 =
        (((($FL[α',l,α]*$A[α,u,β])*$O[l,d,r,u])*conj($A[α',d,β']))*$FR[β,r,β'])
...
"""
    peps1checkalt(T = Float64; D = 4, d = 2, χ = 256)
...
*   χ: boundary MPS bond dimension (range 10 - 1000, the larger the better)
"""
function peps1checkalt(T = Float64; D = 4, d = 2, χ = 256)
    A = randn(T, (D, D, D, D, d))
    FL = randn(T, (χ, D, D, χ))
    FR = randn(T, (χ, D, D, χ))
    FU = randn(T, (χ, D, D, χ))
    ...
    α = rand(T)
    β = rand(T)
    @tensor x1[l',d',r',u',s] := β*A[l',d',r',u',s] +
        α*(FL[ld,l,l',lu]*FU[lu,u,u',ru]*A[l,d,r,u,s]*FR[rd,r,r',ru]*FD[ld,d,d',rd])

    @tensor x2[l',d',r',u',s] := β*cA[l',d',r',u',s] +
        α*(cFL[ld,l,l',lu]*cFU[lu,u,u',ru]*cA[l,d,r,u,s]*cFR[rd,r,r',ru]*cFD[ld,d,d',rd])

    @cutensor x3[l',d',r',u',s] := β*A[l',d',r',u',s] +
        α*(FL[ld,l,l',lu]*FU[lu,u,u',ru]*A[l,d,r,u,s]*FR[rd,r,r',ru]*FD[ld,d,d',rd])

    return x1 ≈ collect(x2) ≈ collect(x3)
end
...
"""
    peps1benchalt(T = Float64; D = 4, d = 2, χ = 256)
...
*   χ: boundary MPS bond dimension (range 10 - 1000, the larger the better)
"""
function peps1benchalt(T = Float64; D = 4, d = 2, χ = 256)
    A = randn(T, (D, D, D, D, d))
    FL = randn(T, (χ, D, D, χ))
    FR = randn(T, (χ, D, D, χ))
    FU = randn(T, (χ, D, D, χ))
    ...
    α = rand(T)
    β = rand(T)
    b1 = @benchmark @tensor x1[l',d',r',u',s] := $β*$A[l',d',r',u',s] +
        $α*($FL[ld,l,l',lu]*$FU[lu,u,u',ru]*$A[l,d,r,u,s]*$FR[rd,r,r',ru]*$FD[ld,d,d',rd])

    b2 = @benchmark CuArrays.@sync @tensor x2[l',d',r',u',s] := $β*$cA[l',d',r',u',s] +
        $α*($cFL[ld,l,l',lu]*$cFU[lu,u,u',ru]*$cA[l,d,r,u,s]*$cFR[rd,r,r',ru]*$cFD[ld,d,d',rd])

    b3 = @benchmark CuArrays.@sync @tensor x3[l',d',r',u',s] := $β*CA($A)[l',d',r',u',s] +
        $α*(CA($FL)[ld,l,l',lu]*CA($FU)[lu,u,u',ru]*CA($A)[l,d,r,u,s]*CA($FR)[rd,r,r',ru]*CA($FD)[ld,d,d',rd])

    b4 = @benchmark CuArrays.@sync @cutensor x4[l',d',r',u',s] := $β*$A[l',d',r',u',s] +
        $α*($FL[ld,l,l',lu]*$FU[lu,u,u',ru]*$A[l,d,r,u,s]*$FR[rd,r,r',ru]*$FD[ld,d,d',rd])
...
function peps2check(T = Float64; D = 4, d = 2, m = 2, χ = 128)
    ...
    @tensor x2 =
        ...
        conj(cA[l'',d'',r'',u'',s'])*cFR[rd,r,r',r'',ru]*cFD[ld,d,d',d'',rd])
